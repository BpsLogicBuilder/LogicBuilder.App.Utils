...
namespace LogicBuilder.App.Utils.Rules
{
    public class RulesLoader(IRulesSerializer rulesSerializer) : IRulesLoader
    {
        ...
        public Task LoadRules(RulesModule module, IRulesCache cache, RulesLoaderRequest rulesLoaderRequest)
        {
            return Task.Run
            (
                () =>
                {
                    ...
                    if (cache.RuleEngines.ContainsKey(moduleName))
                        cache.RuleEngines[moduleName] = new RuleEngine(ruleSet, _rulesSerializer.GetValidation(ruleSet, rulesLoaderRequest));
                    else
                        cache.RuleEngines.Add(moduleName, new RuleEngine(ruleSet, _rulesSerializer.GetValidation(ruleSet, rulesLoaderRequest)));
        ...
        public Task LoadRulesOnStartUp(RulesModule module, IRulesCache cache, RulesLoaderRequest rulesLoaderRequest)
        {
            return Task.Run
            (
                () =>
